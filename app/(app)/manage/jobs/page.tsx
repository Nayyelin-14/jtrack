      ...
      {data && data.jobs.length === 0 && (
        <div className="text-center py-16">
          ...
          <Button asChild className="mt-4 rounded-lg">
            <Link href="/manage/jobs/create">Post a Job</Link>
          </Button>
        </div>
      )}
            ...
            <div key={job.job_id} className="rounded-xl border border-zinc-200 dark:border-zinc-800 p-5 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors group">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <Link href={`/manage/jobs/${job.job_id}`} className="font-semibold text-sm text-zinc-800 dark:text-zinc-100 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                    {job.title}
                  </Link>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link
                    href={`/manage/jobs/${job.job_id}/applications`}
